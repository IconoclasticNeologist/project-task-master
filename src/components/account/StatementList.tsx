...
export function StatementList({
  ...
  const [rows, setRows] = useState<StatementRow[]>(() => listStatements());
  ...
  const refresh = () => setRows(listStatements());

  const onSaveNew = () => {
    if (!draftText.trim()) return;
    upsertStatement({ text: draftText.trim(), visibility: draftVis });
    setDraftText("");
    setDrafting(false);
    refresh();
  };
  ...
  const onSaveEdit = (id: string) => {
    upsertStatement({ id, text: editText.trim(), visibility: editVis });
    setEditingId(null);
    refresh();
  };
